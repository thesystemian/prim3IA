...
draw_header() {
    echo -e "${BORDER}┌──────────────────────────────────────────────────────────┐${NC}"
    echo -e "${BORDER}│${NC}  ${P}${BOLD}🐉 PRIM ORCHESTRATOR v1.5${NC}                           ${BORDER}│${NC}"
    echo -e "${BORDER}├──────────────────────────────────────────────────────────┤${NC}"
}
...
# Improved extraction: NO MORE TRUNCATION
extract_full() {
    local agent_id=$1
    local log=$2
    # Extract everything between markers
    local content=$(sed -n "/\[$agent_id Response\]:/,/---/p" "$log" | sed "1d;\$d")
    if [ -z "$content" ]; then echo "Aucune réponse détectée."; else echo "$content"; fi
}
...
format_output() {
    ...
    case $mode in
        summary)
            ...
            echo -e "${TEXT}$(extract_full "EU" "$log" | sed 's/^/  └─ /')${NC}\n"

            echo -e "  ${A_CN}[CN Response]:${NC}"
            echo -e "${TEXT}$(extract_full "CN" "$log" | sed 's/^/  └─ /')${NC}\n"

            echo -e "  ${A_US}[US Response]:${NC}"
            echo -e "${TEXT}$(extract_full "US" "$log" | sed 's/^/  └─ /')${NC}\n"
...
run_mission() {
    ...
    # PAUSE MECHANISM
    echo -e "\n${HEADER}Press enter to return to prompt...${NC}"
    read
}
...
if [ ! -z "$1" ]; then
    ...
    while true; do
        ...
        case $input in
            ...
            mission\ *) run_mission "$(echo $input | cut -d' ' -f2-)" ;;
